...
type FormValues = {
  ...
  EstimatedAt: string // формат yyyy-MM-dd
  Description?: string
  MainPhoto: FileList
}
...
export default function CreateCompanyUserForm() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    setLoading(true)
    try {
      await axiosUser.post('/Users/add-company-user', formData, {
  ...
  return (
    <div className="px-4 sm:px-6">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 w-full max-w-md mx-auto">
        <h2 className="text-xl font-semibold text-center sm:text-left">Register Company</h2>
        ...
        <div className="grid gap-1">
          <Label>Country</Label>
          <Input required {...register('Country', { required: true })} />
        </div>

        <div className="grid gap-1">
          <Label>Region</Label>
          <Input required {...register('Region', { required: true })} />
        </div>

        <div className="grid gap-1">
          <Label>Settlement</Label>
          <Input required {...register('Settlement', { required: true })} />
        </div>

        <div className="grid gap-1">
          <Label>ZIP Code</Label>
          <Input required {...register('ZipCode', { required: true })} />
        </div>
        ...
        <div className="grid gap-1">
          ...
          <Input
            required
            type="date"
            {...register('EstimatedAt', { required: true })}
          />
        </div>
