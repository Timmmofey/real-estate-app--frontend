...
interface Props {
    formType: "ResetPassword" | "CreatePassword"
}

export default function SetPasswordMultiStepForm({formType}: Props) {
  ...
  const handleSetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCode('')
      setNewPassword('')
      router.push(user ? '/settings': '/login')
    } catch (err: unknown) {
      if (err instanceof AxiosError) {
        console.error(err.response?.data || err.message)
      }
      toast.error(formType == "ResetPassword" ? t("toastResetError") : t("toastCreationError"))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-md w-full mx-auto mt-10 px-4">
      <h1 className="text-2xl font-semibold mb-6 text-center">{formType == "ResetPassword" ? t("resetPasswordTitle") : t("createPasswordTitle")}</h1>

      {step === 1 && (
        <form onSubmit={handleSendCode} className="space-y-4">
          <p className="text-sm text-muted-foreground">{t("sendVerificationCodeHint")}</p>
